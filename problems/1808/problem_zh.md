...
输入的第一行包含两个整数$N$和$M$，分别表示城市和道路的个数。

接下来的M行每行包含3个整数$A$, $B$和$C$, 用来描述一条道路。$A$和$B$是不同的整数，表示由这条道路直接相连的两个城市。对于铺设好的道路$C$是$0$；对于土路，$C$是在该条路上设置路障所需的费用值。
...
## 数据规模与约定
$2\leq N\leq 1000, N-1\leq M\leq 5000$

$1\leq A\leq N, 1\leq B\leq N, 0\leq C\leq 10^4$
